...
input_file = 'calibrate_this.nn'
# FULL INPUT FILE TO WRITE FOR USE
write_file = 'new.input'
...
def initial_guess():
    ...
    # THIS IS THE WRONG SPOT FOR THIS CODE -> MAKING VALUE FOR TYPE 2 THE VALUE FROM TYPE 12
    # if num_elem != 1:
    ...
    if num_elem != 1:
        ...
        if len(np.atleast_1d(elements)) != 1:
            ...
            rc = np.delete(rc,1)
            dr = np.delete(dr,1)
            # print('alpha before delete = ',alpha)
            # print('new_alpha = ',new_alpha)
            alpha = np.delete(alpha,1)
            # print('alpha after delete = ',alpha)
...
#                 return_values.append(re)
    ...
    if params_to_fit[5] == True:
        # print('bnds_params[3] = ',bnds_params[3])
        # print('alpha = ',alpha)
        if num_elem != 1:
